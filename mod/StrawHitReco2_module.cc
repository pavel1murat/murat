...
//
// $Id: StrawHitReco2_module.cc,v 1.12 2014/03/25 22:14:39 brownd Exp $
// $Author: brownd $ 
...
namespace mu2e {
  ...
  class StrawHitReco2 : public art::EDProducer 
  {
     public:
       explicit StrawHitReco2(fhicl::ParameterSet const& pset);
       virtual ~StrawHitReco2(); 
  ...
  StrawHitReco2::StrawHitReco2(fhicl::ParameterSet const& pset) :
  ...
  StrawHitReco2::~StrawHitReco2() {}

  
  //------------------------------------------------------------------------------------------
  void StrawHitReco2::beginJob()
  {
  }

  void StrawHitReco2::beginRun(art::Run& run)
  {    
	 ...
	 pfit_ = std::unique_ptr<TrkHitReco::PeakFit>(new TrkHitReco::PeakFitRoot(*strawele,peakfit_) );
      ...
      if (printLevel_ > 0) std::cout << "In StrawHitReco2 begin Run " << std::endl;
  }



  //------------------------------------------------------------------------------------------
  void StrawHitReco2::produce(art::Event& event)
  {        
      if (printLevel_ > 0) std::cout << "In StrawHitReco2 produce " << std::endl;
...
using mu2e::StrawHitReco2;
DEFINE_ART_MODULE(StrawHitReco2);
