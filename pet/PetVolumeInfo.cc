...
//
// $Id: PetVolumeInfo.cc,v 1.1 2013/06/04 19:20:18 murat Exp $
// $Author: murat $
...
namespace mu2e {
  ...
  PetVolumeInfo::PetVolumeInfo( const std::string&  pName,
  ...
  const CLHEP::Hep3Vector& PetVolumeInfo::mu2eOriginInWorld() {
    ...
    static art::ServiceHandle<PetGeometryService> sg;
    static CLHEP::Hep3Vector _origin(sg->hasElement<PetWorldG4>() ?
                                     PetGeomHandle<PetWorldG4>()->mu2eOriginInWorld()
