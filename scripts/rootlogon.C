...
// 	  gSystem->Getenv("MU2E_TEST_RELEASE"));
...
//-----------------------------------------------------------------------------
    //     gSystem->Load("$MU2E_TEST_RELEASE/lib/libmu2e_Mu2eInterfaces.so");
    //     gSystem->Load("$MU2E_TEST_RELEASE/lib/libmu2e_CalorimeterGeom.so");

    gSystem->Load("$MU2E_TEST_RELEASE/lib/libStntuple_base.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libStntuple_obj.so");
    //     gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_geom.so");
    ...
    //    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_ana.so");


    //    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_gui.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_obj.so");
    //    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_alg.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_ana.so");
  }
  else if (strstr(exec_name,"stnfit.exe") != 0) {
//-----------------------------------------------------------------------------
//  running stnfit.exe
//-----------------------------------------------------------------------------
    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_base.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libStntuple_obj.so");
    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_loop.so");
//     gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_geom.so");
//     gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_alg.so");
//
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libmurat_obj.so");
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libmurat_geom.so");
    if (batch_mode == 1) {
      gSystem->Load("$ROOTSYS/lib/libGui.so");
    }
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libStntuple_gui.so");
    //    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_gui.so");
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libmurat_gui.so");
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libmurat_alg.so");
//
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libStntuple_fit.so");
//    gSystem->Load("$MU2E_TEST_RELEASE/shlib/$BFARCH/libmurat_ana.so");
  }

  else if (strstr(exec_name,"mu2e.exe") != 0) {
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_obj.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libStntuple_val.so");
    gSystem->Load("$MU2E_TEST_RELEASE/lib/libmurat_plot.so");
  }
