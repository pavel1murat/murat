...
//-----------------------------------------------------------------------------
//  efficiency of the R > (kFirstBin-1) cm cut
//
// 1. position the first disk
//-----------------------------------------------------------------------------
void plot_tcalm003_001(const char* Filename = "hist/me000001.tcalm003.hist", int FirstBin = 37) {
  TH1   *h_qtot, *h1, *h2, *h3;
  ...
  for (int i=0; i<np; i++) {
    ...
    x  [i] = 5*i;
    ex[i]  = 0;
    eff[i] = h1->Integral(FirstBin,100)/qtot;
    err[i] = sqrt(h1->Integral(FirstBin,100))/qtot;
...
//-----------------------------------------------------------------------------
  ...
  for (int i=0; i<np; i++) {
    ...
    x   [i] = 5*i;
    ex  [i]  = 0;
    eff2[i] = h1->Integral(FirstBin,100)/qtot;
    err2[i] = sqrt(h1->Integral(FirstBin,100))/qtot;
  }
...
//-----------------------------------------------------------------------------
void plot_tcalm003_002_360(const char* Filename = "hist/me000001.tcalm003.hist", int IPL1) {
  TH1   *h_qtot, *h1, *h2, *h3;
...
//-----------------------------------------------------------------------------
 ...
 //  for (int i1=0; i1<np; i1++) {
  ...
  double eff1 = h0->Integral(kFirstBin,100)/qtot;
  ...
  for (int i2=IPL1+1; i2<np; i2++) {
    ...
    x [i2] = 5*i2;
    ex[i2]  = 0;
    eff[i2] = h1->Integral(kFirstBin,100)/qtot;
    err[i2] = sqrt(h1->Integral(kFirstBin,100))/qtot;

    eff2[i2] = h1->Integral(kFirstBin,100)/qtot  + eff1;
    err2[i2] = sqrt(h1->Integral(kFirstBin,100)+h0->Integral(kFirstBin,100))/qtot;
...
// for each plane plot efficiency for its misses 
//-----------------------------------------------------------------------------
void plot_tcalm003_002_330(const char* Filename = "hist/me000001.tcalm003.hist", int IPL1) {
  TH1   *h_qtot, *h1, *h2, *h3;
...
//-----------------------------------------------------------------------------
 ...
 //  for (int i1=0; i1<np; i1++) {
  ...
  for (int i2=IPL1+1; i2<np; i2++) {
    h1     = get_mu2e_hist(Filename,Form("//TCalm003/Hist/evt_0/rt33_0_%02i_%02i",IPL1,i2));
    x [i2] = 5*i2;
    ex[i2]  = 0;
    eff[i2] = h1->Integral(kFirstBin,100)/qtot;
    err[i2] = sqrt(h1->Integral(kFirstBin,100))/qtot;
...
//-----------------------------------------------------------------------------
// for each plane plot efficiency for its misses R=360
//-----------------------------------------------------------------------------
void plot_tcalm003_003_360(const char* Filename = "hist/me000001.tcalm003.hist") {
  TH1   *h_qtot, *h1, *h2, *h3;
...
// for each plane plot efficiency for its misses R=360
//-----------------------------------------------------------------------------
void plot_tcalm003_003_330(const char* Filename = "hist/me000001.tcalm003.hist") {
  TH1   *h_qtot, *h1, *h2, *h3;

  int const kFirstBin = 34;

  TCanvas* c = new TCanvas("c_plot_tcalm003_003_330","Efficiency vs disk position, no cuts",0,0,1200,500);
  c->Divide(2,1);
...
//-----------------------------------------------------------------------------
 ...
 //  for (int i1=0; i1<np; i1++) {
  ...
  for (int i1=0; i1<40; i1++) {
    ...
    for (int i2=i1+1; i2<np; i2++) {
      h1    = get_mu2e_hist(Filename,Form("//TCalm003/Hist/evt_0/rt33_0_%02i_%02i",i1,i2));
      eff_2 = h1->Integral(kFirstBin,100)/qtot;
