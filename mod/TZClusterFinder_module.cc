...
namespace mu2e {
  ...
  public:
    struct Config {
      ...
      fhicl::Atom<art::InputTag>              chCollTag     {Name("chCollTag"),      Comment("ComboHit coll tag" ) };
      ...
      fhicl::Atom<int>                        testOrder     {Name("testOrder"),      Comment("Test order"        ) }; 
      fhicl::Atom<float>                      minHitEnergy  {Name("minHitEnergy"),   Comment("Minimum hit energy") }; 
      fhicl::Atom<float>                      maxHitEnergy  {Name("maxHitEnergy"),   Comment("Maximum hit energy") }; 
      //      fhicl::Atom<fhicl::ParameterSet>        diagPlugin    {Name("diagPlugin"),     Comment("diag plugin pars  ") }; 
    };
...
//-----------------------------------------------------------------------------
    ...
    TZClusterFinderTypes::Data_t        _data;              // all data used
...
//-----------------------------------------------------------------------------
  TZClusterFinder::TZClusterFinder(const art::EDProducer::Table<Config>& config): art::EDProducer{config},
    _chCollTag      (config().chCollTag   ()),
    _minHitEnergy   (config().minHitEnergy()),
...

  {
    ...
    _testOrderPrinted = 0;
    _tdbuff           = 80.;   // mm ... a bit less than 1 ns

    // if (_diagLevel != 0) _hmanager = art::make_tool  <ModuleHistToolBase>(config().diagPlugin());
...
//-----------------------------------------------------------------------------
  void TZClusterFinder::beginRun(art::Run& aRun) {
...
//-----------------------------------------------------------------------------
  int TZClusterFinder::orderHits() {
    ...
    for (int h=0; h<nhits; ++h) {
      ...
      if (sh->energyDep() < _minHitEnergy)                 continue;
...
//-----------------------------------------------------------------------------
  void  TZClusterFinder::runTZFinder() {
...
//-----------------------------------------------------------------------------
    ...
    _data.tccHolder.clear();
//-----------------------------------------------------------------------------
// process event
//-----------------------------------------------------------------------------
    if (! findData(Event)) {
      const char* message = "mu2e::TZClusterFinder_module::produce: data missing or incomplete";
...
//-----------------------------------------------------------------------------
    ...
    int ntcc = _data.tccHolder.size();

    for (int i=0; i<ntcc; i++) {
      TCCandidate_t* tcc = &_data.tccHolder.at(i);

      // create time peak
      TimeCluster tpeak;

      for (int ih=0; ih<tcc->NComboHits(); ih++) {
	tpeak._strawHitIdxs.push_back(tcc->StrawHitIndex(ih) );
	//	tpeak._nsh              = nsh;
	tpeak._t0               = TrkT0(tcc->Time(), 0.1); //dummy value for errT0
	//tpeak._pos              = tpos;
      }
      tccol->push_back(tpeak);
    }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  void TZClusterFinder::orderID(ChannelID* X, ChannelID* O) {
...
//-----------------------------------------------------------------------------
  void TZClusterFinder::deOrderID(ChannelID* X, ChannelID* O) {
...
//-----------------------------------------------------------------------------
  void TZClusterFinder::testOrderID() {
...
//-----------------------------------------------------------------------------
  void TZClusterFinder::testdeOrderID() {
...
//-----------------------------------------------------------------------------
DEFINE_ART_MODULE(mu2e::TZClusterFinder)


// //-----------------------------------------------------------------------------
//   int TZClusterFinder::findIntersection(const HitData_t* Hd1, const HitData_t* Hd2, Intersection_t* Result) {
...
// //-----------------------------------------------------------------------------
//   void TZClusterFinder::findSeeds(int Station, int Face) {

//     TZClusterFinderTypes::Intersection_t  res;
...
//               //-----------------------------------------------------------------------------
//               TZClusterFinderTypes::findIntersection(hd1,hd2,&res);
//               //-----------------------------------------------------------------------------
...
// //-----------------------------------------------------------------------------
//   void TZClusterFinder::pruneSeeds(int Station) {
...
// //-----------------------------------------------------------------------------
//   int TZClusterFinder::recoverMissingHits() {
...
// //-----------------------------------------------------------------------------
//   void TZClusterFinder::connectSeeds() {
// 
//     //loop over stations
//     //start with a seed
...
// //-----------------------------------------------------------------------------
//   void TZClusterFinder::findSeeds() {
// 
//     for (int s=0; s<kNStations; ++s) { 
...
// //-----------------------------------------------------------------------------
//   void TZClusterFinder::getNeighborHits(DeltaSeed* Seed, int Face, int Face2, PanelZ_t* panelz) {
...
//       TZClusterFinderTypes::findIntersection(hd, hd2, &res);
...
// //-----------------------------------------------------------------------------
//   int TZClusterFinder::recoverStation(DeltaCandidate* Delta, int Station) {
...
// //-----------------------------------------------------------------------------
//   int TZClusterFinder::checkDuplicates(int Station, int Face1, const HitData_t* Hit1, int Face2, const HitData_t* Hit2) {
