...
//
// $Id: GaasTrackingAction.cc,v 1.9 2014/08/29 22:37:41 genser Exp $
// $Author: genser $
...
#include "murat/gaas/GaasTrackingAction.hh"
...
namespace mu2e {
  ...
  GaasTrackingAction::GaasTrackingAction( const SimpleConfig& config,
                                            GaasSteppingAction* steppingAction ):
    _debugList(),
    _physVolHelper(0),
    _timer(),
  ...
  GaasTrackingAction::~GaasTrackingAction(){
  }

  // Receive information that has a lifetime of a run.
  void GaasTrackingAction::beginRun( const PhysicalVolumeHelper& physVolHelper,
  ...
  void GaasTrackingAction::endRun(){
  }

  void GaasTrackingAction::PreUserTrackingAction(const G4Track* trk){
  ...
  void GaasTrackingAction::PostUserTrackingAction(const G4Track* trk){
  ...
  void GaasTrackingAction::beginEvent( art::Handle<GenParticleCollection> const& gensHandle, 
  ...
  void GaasTrackingAction::endEvent(SimParticleCollection& persistentSims ){
  ...
  void GaasTrackingAction::saveSimParticleStart(const G4Track* trk){
    ...
    if( _sizeLimit>0 && _currentSize>_sizeLimit ) {
      if( (_currentSize - _sizeLimit)==1 ) {
        mf::LogWarning("G4") << "Maximum number of particles reached in GaasTrackingAction: "
                              << _currentSize << endl;
  ...
  void GaasTrackingAction::saveSimParticleEnd(const G4Track* trk){
