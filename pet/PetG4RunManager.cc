...
// the event loop.
//
// $Id: PetG4RunManager.cc,v 1.1 2013/06/04 19:20:18 murat Exp $
// $Author: murat $
...
namespace mu2e {
  ...
  PetG4RunManager::PetG4RunManager():
    G4RunManager(),
    _macroFile(0),
    _n_select(-1),
    _nProcessed(0),
  ...
  void PetG4RunManager::BeamOnBeginRun( unsigned int runNumber, const char* macroFile, G4int n_select){
  ...
  void PetG4RunManager::BeamOnDoOneEvent( int eventNumber){
  ...
  void PetG4RunManager::BeamOnEndEvent(){
  ...
  void PetG4RunManager::BeamOnEndRun(){
