...
//
// $Id: PetStackingAction.cc,v 1.1 2013/06/04 19:20:18 murat Exp $
// $Author: murat $
...
namespace mu2e {
  ...
  PetStackingAction::PetStackingAction( const SimpleConfig& config ):
    _ncalls(0),
    _nevents(0),
    ...
    if( _verbose>0 && _pdgToDrop.size()>0 ) {
      cout << "Drop these particles in the PetStackingAction: ";
      for( size_t i=0; i<_pdgToDrop.size(); ++i ) cout << _pdgToDrop[i] << ",";
      cout << endl;
    }

    if( _verbose>0 && _pdgToKeep.size()>0 ) {
      cout << "Keep these particles in the PetStackingAction: ";
  ...
  void PetStackingAction::beginRun( double dirtG4Ymin, double dirtG4Ymax ){
...
//     _dirtBodyPhysVol.push_back(G4PhysicalVolumeStore::GetInstance ()->GetVolume("HallWalls"));
  ...
  PetStackingAction::ClassifyNewTrack(const G4Track* trk){
    ++_ncalls;
  ...
  bool PetStackingAction::cosmicKiller( const G4Track* trk){
  ...
  } // end PetStackingAction::cosmicKiller
