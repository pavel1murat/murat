...
//
// $Id: GaasStudy_module.cc,v 1.9 2013/12/17 21:49:06 genser Exp $
// $Author: genser $
...
namespace mu2e {
  ...
  private:
    ...
    GaasTrackingAction*    _trackingAction;
    GaasSteppingAction*    _steppingAction;
  ...
  void GaasG4::initializeG4( GeometryService& geom, art::Run const& run ){
    ...
    _steppingAction = new GaasSteppingAction(config);
    _runManager->SetUserAction(_steppingAction);

    G4UserEventAction* event_action = new GaasEventAction(_steppingAction);
    _runManager->SetUserAction(event_action);

    _trackingAction = new GaasTrackingAction(config,_steppingAction);
