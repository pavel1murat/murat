...
//
// $Id: PetConstructMaterials.cc,v 1.3 2013/11/05 19:48:43 murat Exp $
// $Author: murat $
// $Date: 2013/11/05 19:48:43 $
//
...
namespace mu2e {
  ...
  PetConstructMaterials::PetConstructMaterials(){
  }
  ...
  void PetConstructMaterials::construct(){
  ...
  void PetConstructMaterials::constructMu2eMaterials(SimpleConfig const& config){
   ...
   // http://personalpages.to.infn.it/~tosello/EngMeet/ITSmat/SDD/E_glass.html
    ...
    mat = isNeeded(materialsToLoad, "BaF2");  /// 2013-10-19 P.Murat
    if ( mat.doit ){
      G4double density;
      G4int    nel, natoms;
      G4Material* BaF2 = new G4Material(mat.name, density = 4.88*g/cm3,nel=2);
      BaF2->AddElement(getElementOrThrow("Ba"),natoms=1);
      BaF2->AddElement(getElementOrThrow("F") ,natoms=2);
    }   

    mat = isNeeded(materialsToLoad, "H2O");  /// 2013-10-31 P.Murat
    if ( mat.doit ){
      G4double density;
      G4int    nel, natoms;
      G4Material* H2O = new G4Material(mat.name, density = 1*g/cm3,nel=2);
      H2O->AddElement(getElementOrThrow("H"),natoms=2);
      H2O->AddElement(getElementOrThrow("O") ,natoms=1);
    }   
  ...
  CheckedG4String PetConstructMaterials::isNeeded( vector<string> const& V, string const& s){
  ...
  bool PetConstructMaterials::isRequested( vector<string> const& V, string const& s){
  ...
  void PetConstructMaterials::uniqueMaterialOrThrow( G4String const& name){
    if ( G4Material::GetMaterial(name,false) != 0 ){
      throw cet::exception("GEOM")
        << "mu2e::PetConstructMaterials::constructMu2eMaterials(): "
  ...
  G4Element* PetConstructMaterials::getElementOrThrow( G4String const& name){
    ...
    if ( !answer ){
      throw cet::exception("GEOM")
        << "mu2e::PetConstructMaterials::constructMaterials(): "
